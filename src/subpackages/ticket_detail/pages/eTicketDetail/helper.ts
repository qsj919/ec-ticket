...
export function getBalance(
  ...
  if (fieldConfig.showThisBalance === '1') {
    ...
    const value = Math.abs(main.balance)
  ...
  if (fieldConfig.showLastBalance === '1') {
    ...
    const value = Math.abs(main.lastBalance)
    result.push({ label, value })
  }
...
export function getOthetrInfo(main: mainType) {
  ...
  const addressArray = main.shopAddrs
    ? main.shopAddrs.map((addr, idx) => ({ label: t('address') + idx > 0 ? idx : '', value: addr }))
