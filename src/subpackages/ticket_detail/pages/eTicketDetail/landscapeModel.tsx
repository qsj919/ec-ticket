...
import { getOneDate, getDateTime, urlQueryParse, objectParseToUrlQuery, getTaroParams } from '@utils/utils'
import { getGoodsDetail, ticketLink, reportScreenCapture } from '@api/apiManage'
import LoginView from '@components/LoginView/LoginView'
import Notice from '../../images/notice_image.png'
import NoticeClose from '../../images/notice_cloce_icon.png'
import NoticePublic from '../../images/notice_public.png'
...
import { CloudSource, CloudVideoSource, CLOUD_BILL_FLAG, ICloudBill, Sku } from '@@types/base'
...
import iconJia from '../../images/icon_jia.png'
import iconYi from '../../images/icon_yi.png'
import iconBing from '../../images/icon_bing.png'
import StoreCustomerQrCodePopUp from '../../components/StoreCustomerQrCodePopUp'
import { getStoreCustomerQrCodeUrl } from '@api/user_api_manager'
// import { getETicketDetail } from './api'
import { fuckmain } from './encrypt'
...
type StateType = {
  ...
  productType?: number
  fromPublic: string
  boss: string // 是否从消息推送公众号进入小票 boss= 1 是
  independentType: number
  independentAppId: string
  noticeIsShow: boolean
  ticketSubscribe: number
  hasRealPrice: boolean // 是否有单价，调拨单可能没有单价
  storeCustomerQrCodePopUpData: {
    visible: boolean
    url: string
  }
}
...
const mapStateToProps = ({ user, replenishment, systemInfo, shop }: GlobalState) => ({
  showPhoneEntry:
    (user.grayFunc.mobileVerify === 1 || user.grayFunc.mobileVerify === 2) &&
    user.id !== -1 &&
    (!user.phone || !user.nickName),
  /**
   * 是否黑名单用户，如是 需要获取其昵称 + 手机号，并限制其切换样式
   */
  isBlockUser: user.grayFunc.mobileVerify === 2,
  noAuth: user.noAuth,
...
class TicketDetailPage extends Component {
  ...
  constructor(props) {
    ...
    let boss = '0'
    if (getTaroParams(Taro.getCurrentInstance?.()).boss) {
      boss = `${getTaroParams(Taro.getCurrentInstance?.()).boss}`
    }
    ...
    this.state = {
      ...
      params: {
        ...
        sn: params.sn,
        type: params.type || '1', // 2订货 1拿货 3连锁调拨单
        fromQR: params.fromQR
      },
  ...
  componentDidMount() {
    ...
    if (this.props.sessionId) {
      myLog.log('got sessionId, start request')
  ...
  componentDidUpdate(prevProps: Readonly<StateProps>) {
    ...
    if (!prevProps.sessionId && this.props.sessionId) {
      myLog.log('previous no session, now got sessionId, start request')
      this.initPageData().then(() => {
        if (getTaroParams(Taro.getCurrentInstance?.()).downloadImage === '1') {
          this.onDownloadClick()
        }
  ...
  initPageData = async () => {
    ...
    if (getTaroParams(Taro.getCurrentInstance?.()).targetUser === '1') {
      getStoreCustomerQrCodeUrl().then(res => {
        if (res.data.val) {
          this.setState({
            storeCustomerQrCodePopUpData: {
              visible: true,
    ...
    const {
      params: { pk, sn, epid, type, mpUserId, fromQR },
      source,
      boss
    } = this.state
    const params: TicketDetailParams = {
      type,
      pk,
      sn,
      epid,
    ...
    // 从消息推送公众号进入&&线上环境  通知服务端加埋点
    if (boss === '1' && process.env.PRODUCT_ENVIRONMENT === 'product') {
      params.boss = 1
    }
    this.billDataFetching = true
    if (fromQR === CloudVideoSource.TICKET_QR) {
      Object.assign(params, { source: 4 })
    }
    try {
      const res = await fuckmain(params)
      ...
      // 根据rowId排序
      const dets: Array<any> = res.data.dets
        .sort((spuA, spuB) => (spuA.rowId || 0) - (spuB.rowId || 0))
        .map(d => ({ ...d }))
      ...
      arr.forEach(arrItem => {
        ...
        if (arrItem[0].totalSmall) {
          const totalSmall = arrItem.reduce((sum, item) => {
            return NP.plus(sum, item.totalSmall || 0)
          }, 0)
        ...
        while (arrItem.length > 0) {
          ...
          const tempArrItem = {
            ...
            prodDates: arrItem.map(ar => ({
              prodDate: ar.prodDate,
              price: ar.price,
              stdprice1: ar.stdprice1,
              unit: ar.unit,
              num: ar.num
            })),
            spec: arrItem[0].spec,
            chrgitmLv: arrItem[0].chrgitmLv
          }
          // 生成合计数据
          const sumArr: any = []
          let sizeMapArr: any = []
      ...
      params.sn = res.data.sn
      params.epid = res.data.epid
      params.pk = res.data.billid

      let hasRealPriceFlag = rawDets[0] && rawDets[0].realPrice !== undefined
      this.setState({
        cloudBillFlag,
        userActivityState,
        activityId,
        ...
        independentType: res.data.independentType,
        independentAppId: res.data.independentAppId,
        noticeIsShow: res.data.saveNoticeFlag === 1,
        ticketSubscribe: res.data.subscribe || 0,
        hasRealPrice: hasRealPriceFlag
      })
      this.ticketType = res.data.type
      ...
      myLog.log(`initpage error: ${e.message || e}`)
      this.billDataFetching = false
    }
  }
  ...
  onUserCaptureScreen = () => {
    ...
    reportScreenCapture({ deviceType: platform, sn: params.sn })

    this.setClipboardData('toast')
    trackSvc.track(events.screenCapture)
  }

  setClipboardData = (messageType?: 'toast' | 'alert') => {
    const {
      params: { pk, sn, epid, type }
    } = this.state
    Taro.setClipboardData({ data: `et?sn=${sn}&epid=${epid}&billid=${pk}&type=${type}?et` })
    if (messageType === 'toast') {
      messageFeedback.showToast('口令复制成功，打开商陆花或笑铺日记即刻入库')
    } else {
      messageFeedback.showAlert('口令复制成功，现在打开商陆花或笑铺日记就可以入库啦～')
    }
  }
  ...
  goCloudBillPage = (type: ICloudBill = ICloudBill.all) => {
    if (this.state.independentType !== 2) {
      this.props.dispatch({
        type: 'cloudBill/init',
      ...
      Taro.navigateToMiniProgram({
        appId: this.state.independentAppId
      })
    }
  }
  ...
  onNoticeClick = () => {
    const { ticketSubscribe } = this.state
    const {
      params: { sn, epid, pk }
    } = this.state
    ticketLink({
      ...
      .then(() => {
        if (ticketSubscribe === 0) {
          if (process.env.PRODUCT_ENVIRONMENT === 'product') {
            Taro.previewImage({
              urls: [images.discovery.eticket_focus_public_prod]
            })
          } else {
            Taro.previewImage({
              urls: [images.discovery.eticket_focus_public_test]
            })
          }
        } else {
          this.setState({
  ...
  renderMedicalDets = () => {
    const { dets, main, fieldConfig, sameCodeObjArr, config, privateFlag, productType } = this.state
    return (
      <View className={styles.listForm}>
        {dets.map((item, index) => (
          <View key={index} className={styles.listItem}>
            <View className={styles.medical_header}>
              <View>
                {this.getIcon(item[0].chrgitmLv) && (
                  <Image className='medical_header__icon' src={this.getIcon(item[0].chrgitmLv)} />
                )}
                {`${item[0].name}  ${item[0].spec}`}
              </View>
              <View>
                <Text style={{ fontSize: pxTransform(22) }}>¥</Text>
                {NP.times(item[0].totalNum, item[0].realPrice)}
              </View>
            </View>
            <View
              className={styles.medical_info}
            >{`¥${item[0].realPrice}x${item[0].totalNum}${item[0].unit}`}</View>
          </View>
        ))}
      </View>
    )
      ...
      <View className={styles.listForm}>
        {dets.map((item, index) => (
          <View key={index} className={styles.listItem}>
                ...
                >
                    ...
                    {!notShowMoney && !notShowNum && (
                      <View className={styles.goodsTopRight}>
                        ...
                        <View className={styles.itemTotalPrice}>
                          {sameCodeObjArr[index].totalSmall ||
                            NP.times(
                              sameCodeObjArr[index].sumNum || 0,
                              sameCodeObjArr[index].realPrice || 0
                            )}
                        </View>
                  ...
                  {config.salesPrintMatName === '1' && (
                    <Text selectable className={styles.goodsName}>
                      {item[0].name} {productType === 40 ? item[0].spec : ''}
                    </Text>
                  )}
                  {(productType !== 40 || config.enableStockBatch === '0') && (
                    <View className={styles.goodsPrice}>
                      <Text className={styles.priceNum}>
                        {/* 价格 */}
                        <Text>
                          {`${notShowMoney || notShowNum ? '' : `${item[0].realPrice}元`}`}
                          {!notShowMoney &&
                        ...
                        <Text style={{ marginLeft: '15px' }}>
                          {`${
                            sameCodeObjArr[index].discount !== 1 &&
                            config.shareBillShowDiscount !== '0'
                              ? `(折扣: ${sameCodeObjArr[index].discount})`
                              : ''
                          }`}
                        </Text>
                      </Text>
            ...
            {productType === 40 && config.enableStockBatch === '1' && item[0].flag !== 2 && (
              <Block>
                {item[0].prodDates.map(de => (
                  <View className={styles.grainAndOil}>
                    {/* <Text className={styles.grainAndOil__prodDate}>
                      {de.prodDate && `生产日期: ${de.prodDate}`}
                    </Text> */}
                    <Text className={styles.grainAndOil__priceNum}>
                      {/* 价格 */}
                      <Text>
                        {`${notShowMoney || notShowNum ? '' : `${de.price}元`}`}
                        {!notShowMoney && main.needShowOldPrice && de.stdprice1 > de.price && (
                          <Text className={styles['line_t']}>{`(¥${de.stdprice1})`}</Text>
                        )}
                      </Text>
                      {/* 件数 */}
                      <Text>{`${notShowNum ? '' : ` × ${de.num}${de.unit || ''}`}`}</Text>
                      {/* 折扣 */}
                      <Text>
                        {`${
  ...
  // 平铺展示所有sku数据
  renderDetsInSkus = () => {
    const { rawDets, fieldConfig, config, main, productType, hasRealPrice } = this.state
    ...
    const detsArray = rawDets.reduce((prev, cur, index) => {
      ...
      if (productType === 40) {
        specArr.push(cur.spec || '')
        unitArr.push(cur.unit || '')
      }
      ...
      prev[7].push(cur.totalSmall || String(NP.times(cur.num || 0, cur.realPrice || 0)))
    ...
    // 不显示货号
    if (fieldConfig.showCode && fieldConfig.showCode !== '1' && productType === 40) {
      detsArray.splice(1 - deleteCount, 1)
      deleteCount++
    }
    ...
    // 不显示颜色尺码
    if (config.ignoreColorSize === '1' || productType === 40) {
    ...
    // 以下为食品版逻辑
    if (productType === 40) {
      let detsCodeIndex = detsArray.findIndex(s => s[0] === '款号')
      if (detsArray[detsCodeIndex]) {
        detsArray[detsCodeIndex][0] = '货号'
      }
      let detsNameIndex = detsArray.findIndex(s => s[0] === '名称')
      detsArray.splice(detsNameIndex + 1, 0, specArr)
      detsArray.splice(detsNameIndex + 2, 0, unitArr)
    }

    // 没有单价时隐藏单价和小计
    if (!hasRealPrice) {
      for (let i = 0; i < detsArray.length; i++) {
        if (detsArray[i].includes('单价') || detsArray[i].includes('小计')) {
          detsArray.splice(i, 1)
          i--
        }
      }
    }
  ...
  renderDetails = () => {
    ...
    const isMedicalIndustry = industryId === 10 && productType === 20
    const isBothSaleAndBackGoods = main.backNum !== 0 && main.totalNum + main.backNum !== 0
    const renderScore =
      typeof main.compScore !== 'undefined' && config.printClientScore === '1' && !isMedicalIndustry
    ...
    const isTransferBill = params.type === '3'

    /**
     * 本单余额/欠款
     */
    const balanceNum = Math.abs(main.balance)
    const renderBalanceNum = !isTempBill && fieldConfig.showThisBalance === '1'
    /**
     * 上次余额/欠款
     */
    const lastTimeBalanceNum = Math.abs(
      NP.minus(
        main.lastBalance || 0,
        -main.verifySum || 0,
        Number(saasType) !== 1 && main.storedValueCost !== 0
          ? main.balance - main.storedValueCost || 0
          : main.balance || 0
      )
    )
    const renderLastTimeBalanceNum = !isTempBill && fieldConfig.showLastTimeBalance === '1'
    /**
     * 累计余额/欠款
     */
    const lastBalance = Math.abs(main.lastBalance)
    const renderLastBalance = !isTempBill && fieldConfig.showLastBalance === '1'
          ...
          <Block>
            ...
            <View className={styles.total}>
              ...
              <View className={styles.totalBot}>
                {notShowNum
                  ? '**'
                  : `${main.totalNum}${typeof productType === 'undefined' ? t('unit') : ''}`}
              </View>
            </View>
            ...
            {isTransferBill && (
              <View className={styles.total}>
                <View className={styles.shark}>备注</View>
                <View className={styles.remBot}>{main.rem || '无'}</View>
              </View>
            )}
          </Block>
          {/* {!isDetailOtherInfoVisible && (
          ...
          <Block>
            ...
            {boss === '0' && (
              <View className={styles.detailInfo}>
                ...
                {/* printFooter */}
                {!!main.createdDate && !isMedicalIndustry && (
                  <View className={styles.detailItem}>
                    <View>开单时间</View>
                    <Text selectable className={styles.detail}>
                      {main.createdDate}
  ...
  renderDownloadImages = () => {
    const { downloadImages, fieldConfig, isImagesMoreThanOne, boss } = this.state
    return (
      fieldConfig.showImage === '1' &&
      boss === '0' &&
      isImagesMoreThanOne && (
  ...
  renderCloudBillGoods = () => {
    ...
    return renderNothing ? null : boss === '1' ? null : (
      <View
  ...
  renderQRcode = () => {
    const { params, qrUrl, saasType, isVideoVisible, main, boss } = this.state
  ...
  renderBackView = () => {
    ...
    return (
      ...
      >
        ...
        {!isTransferBill && <Text>{type === '1' ? '全部小票' : '全部订货单'}</Text>}
      </View>
    )
  }
  ...
  renderNoticeView = () => {
    return (
      <View className={styles.notice_view} onClick={this.onNoticeClose}>
        <View className={styles.notice_view__content} onClick={e => e.stopPropagation()}>
          <Image src={Notice} style='width:100%;height:100%;' mode='aspectFill' />
          <Image
            src={NoticeClose}
            className={styles.notice_view__content__close}
            onClick={this.onNoticeClose}
          />
          <View className={styles.notice_view__content__action} onClick={this.onNoticeClick}>
            保存小票
          </View>
        </View>
      </View>
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <StoreCustomerQrCodePopUp
          visible={storeCustomerQrCodePopUpData.visible}
          url={storeCustomerQrCodePopUpData.url}
          onCloseClick={() =>
            this.setState({
              storeCustomerQrCodePopUpData: {
                visible: false,
        ...
        <View className={styles.landscapeWrapper} id='landscape-wrapper'>
          {privateFlag === '1' ? (
            ...
            <Block>
              ...
              {/* 下载图片按钮 */}
              {isFunctionAvailable && !isMedicalIndustry && boss === '0' && (
                <View
              ...
              {!isTransferBill && productType !== 40 && !isMedicalIndustry && boss === '0' && (
                <View
                ...
                >
                  <Text style={{ fontSize: '22rpx' }}>商陆花笑铺入库</Text>
                </View>
              )}
              ...
              <View className={styles.ticketTop} style={{ marginTop: pxTransform(offsetTop) }}>
                ...
                <View className={classNames(styles.topRight, styles[`topRight--${params.type}`])}>
                  {!isMedicalIndustry && !isTransferBill && !isBlockUser && (
                    <Button
              ...
              <View className={styles.baseInfo}>
                ...
                <View className={styles.baseInfoItem}>
                  <View className={styles.itemTitle}>
                    {isTransferBill ? '调入门店' : isMedicalIndustry ? '患者' : t('client')}
                  </View>
                  <View
                ...
                <View className={styles.baseInfoItem}>
                  <View className={styles.itemTitle}>
                    {isTransferBill ? '调出门店' : isMedicalIndustry ? '操作人' : t('clerk')}
                  </View>
                  <View className={styles.itemBot}>
                    {isTransferBill ? main.shopName || '-' : main.ownerName ? main.ownerName : '-'}
                  </View>
                </View>
              ...
              {isMedicalIndustry && (
                <View className={styles.detailInfo} style={{ marginBottom: pxTransform(22) }}>
                  {!!main.createdDate && (
                    <View className={styles.detailItem}>
                      <View>诊断时间</View>
                      <Text selectable className={styles.detail}>
                  ...
                  <View className={styles.detailItem}>
                    <View style={{ flexShrink: 0, marginRight: '24px' }}>诊断信息</View>
                    <View>{main.mainCondDscr}</View>
                  </View>
                </View>
              )}
              {isMedicalIndustry && this.renderMedicalDets()}
              {!isMedicalIndustry &&
                ((newStyle === '1' && !isBlockUser) || isTransferBill
                  ? this.renderDetsInSkus()
                  : this.renderDetsInHeadSize())}
              {!fromAppShare && this.renderVerifyDets()}
              {/* 汇总信息 */}
              {!isTransferBill && !isMedicalIndustry && fromAppShare ? (
                ...
                <Block>
                  {this.renderDetails()}
                  {!isTransferBill && this.renderDownloadImages()}
                  {!isTransferBill && this.renderQRcode()}
                  {!isTransferBill && this.renderCloudBillGoods()}
                </Block>
              )}
              {/* <Block>
                {this.renderDetails()}
          ...
          <LoginView
            notStickyTop
            fromTicket
            scanError={showPhoneEntry && (fromPublic || isBlockUser) ? 6 : 0}
          />
        ...
        {noticeIsShow && this.renderNoticeView()}
      </CustomNavigation>
    )
  }
}
